...
void FillTime(TH1* histo, vector<double> q, vector<double> tau)
{
  TRandom3 gen;
  gen.SetSeed(0);

  for (Long64_t iter = 0; iter < 1e6; ++iter)
    {  
      double prob(0);
...
void runTest()
{
  vector<double> qBeta = {0.933, 0.024, 0.022, 0.020};
  vector<double> tauBeta = {3.95, 23.56, 78.86, 546.39};
  
  vector<double> qAlpha = {0.679, 0.144, 0.102, 0.075};
  vector<double> tauAlpha = {4.15, 19.90, 99.91, 617.96};
}
